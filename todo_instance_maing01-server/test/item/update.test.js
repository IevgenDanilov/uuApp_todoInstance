...
describe(`Testing the ${CMD} uuCmd...`, () => {
  test("HDS", async () => {
    ...
    let result = await TestHelper.executePostCommand(CMD, { ...itemCreate, id: resultItem.data.id }, session);
  ...
  test("A3.1 - Item does not exist", async () => {
    ...
    expect.assertions(3);
    try {
      await TestHelper.executePostCommand(CMD, { ...DEFAULT_DTOIN, listId: resultCreate.data.id }, session);
    } catch (e) {
      expect(e.status).toEqual(400);
      expect(e.code).toEqual(expectedError.code);
      expect(e.dtoOut.uuAppErrorMap[expectedError.code].message).toEqual(expectedError.message);
  ...
  test("A4.1 - List does not exist", async () => {
    ...
    expect.assertions(3);
    try {
      await TestHelper.executePostCommand(CMD, { ...DEFAULT_DTOIN, id: resultItem.data.id }, session);
    } catch (e) {
      expect(e.status).toEqual(400);
      expect(e.code).toEqual(expectedError.code);
      expect(e.dtoOut.uuAppErrorMap[expectedError.code].message).toEqual(expectedError.message);
