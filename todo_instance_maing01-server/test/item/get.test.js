...
describe(`Testing the ${CMD} uuCmd...`, () => {
  test("HDS", async () => {
    ...
    let result = await TestHelper.executeGetCommand(CMD, { id: resultItem.data.id }, session);
  ...
  test("A2.1 - TodoInstanceDoesNotExist.", async () => {
    ...
    expect.assertions(4);
    try {
      await TestHelper.executeGetCommand(CMD, DEFAULT_DTOIN, session);
    } catch (e) {
      expect(e.status).toEqual(400);
      expect(e.code).toEqual(expectedError.code);
      expect(e.dtoOut.uuAppErrorMap[expectedError.code].message).toEqual(expectedError.message);
  ...
  test("A4.1 - Item does not exist", async () => {
    let expectedError = {
